...
public class RadixSort : Algorithm {
    ...
    protected IEnumerator _CountingSort(int exp) {
        foreach (var e in inputData) {
            e.Select(true);
            yield return _WaitStep();

            int index = ((e.Value / exp) % 10);
            histData[index].Select(true);
            histData[index].Value++;
            yield return _WaitStep();

            e.Select(false);
            histData[index].Select(false);
        }
        ...
        for (int i = inputData.Length - 1; i >= 0; i--) {
            ...
            int index = ((inData.Value / exp) % 10);
            var htData = histData[index];
    ...
    protected IEnumerator _ResetHist() {
        foreach (var e in histData) {
            e.Select(true);
            yield return _WaitStep();
            e.Value = 0;
    ...
    protected override IEnumerator _Run() {
        yield return _CountingSort(1);
        yield return _Copy(outputData, inputData);
        yield return _ResetHist();
        yield return _CountingSort(10);
